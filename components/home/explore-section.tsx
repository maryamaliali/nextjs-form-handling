  ...
  return (
    <section className="py-14 md:py-20">
      <div className="msa-container">
        ...
        <ul className="msa-explore-grid mx-auto mt-10 grid max-w-4xl gap-3 sm:grid-cols-2 lg:grid-cols-2">
          {explore.cards.map((card, index) => (
            <li key={card.path}>
              <SectionReveal variant="up" delay={index * 90}>
                <Link
                  href={`${base}${card.path}`}
                  className="group msa-card-lift msa-explore-card flex h-full gap-4 rounded-2xl border border-border bg-card p-4 transition hover:border-primary/40 hover:shadow-sm sm:p-5"
                >
                  <span className="msa-explore-card-icon flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary transition-transform">
                    <ExploreIcon name={card.icon as ExploreIconName} />
                  </span>
                  <span className="min-w-0 text-left">
                    <span className="msa-explore-card-title block font-semibold text-foreground group-hover:text-primary">
                      {card.title}
                    </span>
